...
var encodedClientHelloMsg = []byte{
	// Record Header
	0x16,
	0x03, 0x04,
	0x00, 0x9c,
...
var encodedServerHelloMsg = []byte{
	// Record Header
	0x16,
	0x03, 0x04,
	0x00, 0x5b,
	...
	0x00, 0x1d,
	0x00, 0x20,
	0x35, 0x80, 0x72, 0xd6, 0x36, 0x58, 0x80, 0xd1, 0xae, 0xea, 0x32, 0x9a, 0xdf, 0x91, 0x21, 0x38, 0x38, 0x51, 0xed, 0x21,
...
func TestEncodeClientHello(t *testing.T) {
	...
	encoded, err := encodeClientHello([]string{"test.host"}, keys, [32]byte([]byte("some_secret_32_bytes_12345678910")))
...
func TestDecodeClientHello(t *testing.T) {
	...
	assert.Len(t, helloMsg.extensionServerName.entriesList, 1)
	assert.Equal(t, "test.host", string(helloMsg.extensionServerName.entriesList[0].host))

	assert.Len(t, helloMsg.extensionSupportedGroups.groups, 1)
	assert.Equal(t, x25519, helloMsg.extensionSupportedGroups.groups[0])

	assert.Len(t, helloMsg.extensionSignatureAlgorithms.algorithms, 1)
	assert.Equal(t, rsaPssRsaeSha512, helloMsg.extensionSignatureAlgorithms.algorithms[0])

	assert.Len(t, helloMsg.extensionSupportedTLSVersions.versions, 1)
	assert.Equal(t, tls13, helloMsg.extensionSupportedTLSVersions.versions[0])

	assert.Len(t, helloMsg.extensionPSKKeyExchangeModes.modes, 1)
	assert.Equal(t, pskWithECDHEKeyEstablishment, helloMsg.extensionPSKKeyExchangeModes.modes[0])
...
func TestEncodeServerHello(t *testing.T) {
	...
	encoded, err := encodeServerHello(key, [32]byte([]byte("some_secret_32_bytes_12345678910")), tlsAes128GcmSha256)
	require.NoError(t, err)
...
func TestEncodeECPointFormatsExtension(t *testing.T) {
	encoded := encodeECPointFormatsExtension()
	assert.Equal(t, []byte{
		0x0, 0x0b,
		0x0, 0x04,
		0x03,
...
func TestSupportedGroupExtension(t *testing.T) {
	encoded := encodeSupportedGroupsExtension([]supportedKeyExchangeGroup{x25519})
	assert.Equal(t, []byte{
		0x00, 0x0a,
		0x00, 0x04,
		0x00, 0x02,
...
func TestSignatureAlgorithmsExtension(t *testing.T) {
	encoded := encodeSignatureAlgorithmsExtension([]signatureAlgorithm{rsaPssRsaeSha512})
	assert.Equal(t, []byte{
		0x0, 0x0d,
		0x0, 0x04,
		0x0, 0x02,
...
func TestSupportedVersionsExtension(t *testing.T) {
	encoded := encodeSupportedVersionsExtension([]supportedTLSVersion{tls13})
	assert.Equal(t, []byte{
		0x0, 0x2b,
		0x0, 0x03,
		0x02,
...
func TestKeyShareExtension(t *testing.T) {
	encoded := encodeClientKeyShareExtension([]publicKey{{
		payload: [32]byte{
			0x35, 0x80, 0x72, 0xd6, 0x36, 0x58, 0x80, 0xd1, 0xae, 0xea, 0x32, 0x9a, 0xdf, 0x91, 0x21, 0x38,
			0x38, 0x51, 0xed, 0x21, 0xa2, 0x8e, 0x3b, 0x75, 0xe9, 0x65, 0xd0, 0xd2, 0xcd, 0x16, 0x62, 0x54,
		},
